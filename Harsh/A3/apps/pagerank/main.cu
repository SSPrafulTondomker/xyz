...
void print_output(const char *filename, foru *hdist, foru *dist, Graph graph)
{
  ...
  for(int i = 0; i < graph.nnodes; i++) {
    fprintf(o, "%d: %0.6f\n", i, (float)((0.85)*(hdist[i]/300))+(0.15/(float)graph.nnodes));
  }
...
int main(int argc, char *argv[]) {
	...
	//hgraph.optimize();
cudaSetDevice(3);
long long int iii=hgraph.nnodes;
printf("%lld\n",iii );
	...
	pagerank(hdist, dist, graph, totalcommu);

	cudaMemcpy(nerr, &intzero, sizeof(intzero), cudaMemcpyHostToDevice);
	kconf.setMaxThreadsPerBlock();
	
	print_output("obtained_output.txt", hdist, dist, graph);
